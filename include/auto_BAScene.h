...
#ifndef AUTO_BASCENE_H
#define AUTO_BASCENE_H
...
class auto_BAScene{
public:
    auto_BAScene(Eigen::Matrix<double,2,1> obv, Eigen::Matrix<double,3,1> ray, const Eigen::Matrix<double,3,3> K, const double Scale_S_x, const double Scale_S_y):
        _u(obv(0)), _v(obv(1)), _ray(ray), _fx(K(0,0)), _fy(K(1,1)), _cx(K(0,2)), _cy(K(1,2)), _K(K), ScaleScene_x(Scale_S_x), ScaleScene_y(Scale_S_y) {}
    ...
    static ceres::CostFunction *Create(Eigen::Matrix<double,2,1> obv, Eigen::Matrix<double,3,1> ray, const Eigen::Matrix<double,3,3> K, const double Scale_S_x, const double Scale_S_y) {
        return (new ceres::AutoDiffCostFunction<auto_BAScene, 2, 4, 3, 4, 3, 1>(
                    new auto_BAScene(obv, ray, K, Scale_S_x, Scale_S_y)));
    }
